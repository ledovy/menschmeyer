...
if ( ! function_exists( 'mensch_meyer_setup' ) ) :
	...
	function mensch_meyer_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'mensch-meyer' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'mensch_meyer_custom_background_args',
				array(
...
function mensch_meyer_content_width() {
	...
	$GLOBALS['content_width'] = apply_filters( 'mensch_meyer_content_width', 640 );
}
...
function mensch_meyer_widgets_init() {
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'mensch-meyer' ),
...
function mensch_meyer_scripts() {
	wp_enqueue_style( 'mensch-meyer-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'mensch-meyer-style', 'rtl', 'replace' );

	wp_enqueue_script( 'mensch-meyer-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
